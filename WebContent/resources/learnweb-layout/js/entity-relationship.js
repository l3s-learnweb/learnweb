...
function connectedNodes(d) {
    if (toggle === 0) {
        ...
        nodes.style("opacity", function (o) {
            return neighbors.includes(o.node) ? 1 : 0.1;
        });
        edges.style("opacity", function (o) {
            return selectedNode === o.source.node || selectedNode === o.target.node ? 1 : 0.1;
        });
        toggle = 1;
    }
}
...
function recoverNodes() {
    ...
    nodes.style("opacity", function (e) {
        if (e.data.type === 'query') {
            var isIncludes = e.data.categories ? Object.keys(e.data.categories).some(function (cat) {
                return window.categories.includes(cat)
            }) : false;
            ...
            if (neighbors.length) {
                neighborsIncludes = Array.from(neighbors).some(function (neighbor) {
                    ...
                    if (node.categories && Object.keys(node.categories).some(function (cat) {
                        return window.categories.includes(cat)
                    })) {
                        return true;
                    }
