...
public class SearchLogClient
{
    ...
    public void passResultset(LinkedList<ResourceDecorator> resources, MODE mode) throws IOException
    {
        ...
        for(ResourceDecorator resource : resources)
        {
            ...
            if(mode != MODE.text)
                resourceSend = new Resource(resource.getResource().getId(), resource.getUrlReal(), resource.getResource().getType(), resource.getResource().getSource(), resource.getTitle(), StringHelper.shortnString(resource.getDescription(), 1000),
                ...
                resourceSend = new Resource(resource.getResource().getId(), resource.getUrlReal(), resource.getResource().getType(), resource.getResource().getSource(), resource.getTitle(), StringHelper.shortnString(resource.getDescription(), 1000), resultsetId,
                        resource.getTempId());
    ...
    public void saveSERP(int page, MODE mode, LinkedList<ResourceDecorator> serp)
    {
        ...
        if(!resourcesList.containsKey(pageKey) && serp.size() > 0)
        {
            for(ResourceDecorator resource : serp)
            {
                ...
                resourcesResultset.put(resource.getUrlReal(), resourceSend);
            }
