    ...
    public void saveYovistoResource() throws SQLException
    {
        ...
        while(result.next())
        {
            ...
            if(VideoDoesNotExist(result.getInt("yovisto_id")))
            {
                ...
                //Delete the resource if it exists in learnweb
                if(result.getInt("resource_id") != 0)
                {
                    ResourceManager rm = new ResourceManager(learnweb);
                    Resource resource = rm.getResource(result.getInt("resource_id"));
                    rm.deleteResource(resource.getId());
                }
                continue;
            }
