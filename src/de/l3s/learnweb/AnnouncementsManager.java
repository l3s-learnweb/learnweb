...
public class AnnouncementsManager {
    ...
    private void resetCache() throws SQLException {
        ...
        try (PreparedStatement preparedStatement = learnweb.getConnection().prepareStatement(
            "SELECT * FROM lw_news WHERE hidden = false ORDER BY created_at DESC limit ?")) {
    ...
    private void insert(Announcement announcement) throws SQLException {
        try (PreparedStatement stmt = learnweb.getConnection().prepareStatement(
            "INSERT INTO lw_news (title, message, user_id, created_at, hidden) VALUES (?, ?, ?, ?, ?)")) {
