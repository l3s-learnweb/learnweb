...
/* global POSTAGGER_LEXICON, LEXICON_TAG_MAP */
...
class POSTagger {
  ...
  tag(words) {
    const ret = new Array(words.length);
    for (let i = 0, size = words.length; i < size; i++) {
      let ss = this.lexicon[words[i]];
      // 1/22/2002 mod (from Lisp code): if not in hash, try lower case:
      if (!ss) {
    ...
    return Array.from(words).map((word, i) => [word, ret[i]]);
  }

  // eslint-disable-next-line class-methods-use-this
  prettyPrint(taggedWords) {
    Array.from(taggedWords).forEach((word, i) => {
