...
public interface UserDao extends SqlObject, Serializable {
    ...
    @SqlQuery("SELECT u.* FROM lw_user u JOIN lw_group_user USING(user_id) WHERE group_id = ? AND deleted = 0 ORDER BY join_time LIMIT ?")
    ...
    default void save(User user) {
        ...
        SqlHelper.setBitSet(params, User.Guide.values().length, "guide_field", user.getGuides());
    ...
    class UserMapper implements RowMapper<User> {
        ...
        public User map(final ResultSet rs, final StatementContext ctx) throws SQLException {
            ...
            if (user == null) {
                ...
                user.setGuides(SqlHelper.getBitSet(rs, User.Guide.values().length, "guide_field"));
